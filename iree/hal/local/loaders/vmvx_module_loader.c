...
#define IREE_VMVX_ENTRY_SIGNATURE "0rriiiiiiiii_v"
...
static iree_status_t iree_hal_vmvx_executable_issue_call(
  ...
  if (scratchpad_size <= IREE_VMVX_MAX_STACK_SCRATCHPAD_SIZE) {
    scratch_memory =
  ...
  iree_vmvx_interface_t interface;
  iree_vmvx_interface_initialize(scratch_memory, &interface);
  interface.push_constant_count = dispatch_state->push_constant_count;
  interface.push_constants = dispatch_state->push_constants;

  // Interface bindings mapped into on-stack VMVX buffers.
  interface.binding_count = dispatch_state->binding_count;
  interface.bindings = (iree_vmvx_buffer_t*)iree_alloca(
      dispatch_state->binding_count * sizeof(*interface.bindings));
  for (iree_host_size_t i = 0; i < dispatch_state->binding_count; ++i) {
    iree_vmvx_buffer_t* binding = &interface.bindings[i];
    iree_atomic_ref_count_init(&binding->ref_object.counter);
    binding->ptr = dispatch_state->binding_ptrs[i];
    binding->length = dispatch_state->binding_lengths[i];
    // TODO(benvanik): executable layout contains this; though we may want to
    // accelerate the lookup.
    binding->access =
        IREE_VMVX_BUFFER_ACCESS_READ | IREE_VMVX_BUFFER_ACCESS_WRITE;
  }
  ...
  //    )
  //
  // NOTE: this is somewhat of a layering violation, but this is extremely
  // tied to the VM (it's **VM**VX afterall) so I'm fine with it.
  struct {
    iree_vm_ref_t interface;
    ...
    uint32_t workgroup_count_z;
      ...
      .interface =
          {
              .type = iree_vmvx_interface_type_id(),
              .ptr = &interface,
              .offsetof_counter = 0,
          },
      .scratchpad =
          {
              .type = iree_vmvx_buffer_type_id(),
              .ptr = &interface.scratch_buffer,
              .offsetof_counter = 0,
          },
  ...
  iree_vmvx_interface_deinitialize(&interface);

...
static iree_status_t iree_hal_vmvx_driver_factory_try_create(
  ...
  if (IREE_HAL_VMVX_WORKER_COUNT > 0) {
    iree_task_topology_initialize_from_group_count(IREE_HAL_VMVX_WORKER_COUNT,
                                                   &topology);
  } else {
    iree_task_topology_initialize_from_unique_l2_cache_groups(
        /*max_group_count=*/IREE_HAL_MAX_VMVX_WORKER_COUNT, &topology);
  }
...
iree_hal_vmvx_driver_module_register(iree_hal_driver_registry_t* registry) {
  static const iree_hal_driver_factory_t factory = {
      /*self=*/NULL,
      iree_hal_vmvx_driver_factory_enumerate,
      iree_hal_vmvx_driver_factory_try_create,
  };

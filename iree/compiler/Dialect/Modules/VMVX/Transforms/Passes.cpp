...
static llvm::cl::opt<bool> clEnableVectorization(
    "iree-vmvx-vectorize",
    llvm::cl::desc("Enable vectorization (vs scalar fallback)."),
    llvm::cl::init(true));
...
// NOTE:
// NOTE:    this entire pipeline needs to be reworked - it's random
// NOTE:
...
static void buildVectorVMVXTransformPassPipeline(OpPassManager &passManager) {
  ...
  nestedModulePM.addNestedPass<FuncOp>(createFlattenMemRefSubspanPass());
  nestedModulePM.addPass(createNormalizeMemRefsPass());
  nestedModulePM.addNestedPass<FuncOp>(createMemRefDataFlowOptPass());
}

static void buildScalarVMVXTransformPassPipeline(OpPassManager &passManager) {
  OpPassManager &nestedModulePM = passManager.nest<ModuleOp>();
...
void buildVMVXTransformPassPipeline(OpPassManager &passManager) {
  ...
  if (clEnableVectorization) {
    buildVectorVMVXTransformPassPipeline(passManager);
  } else {
    buildScalarVMVXTransformPassPipeline(passManager);
  }
  passManager.addPass(createCanonicalizerPass());

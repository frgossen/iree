...
class ConvertVMVXBufferLoadOp
    : public VMVXImportOpConversion<IREE::VMVX::BufferLoadOp> {
 public:
  using VMVXImportOpConversion<
      IREE::VMVX::BufferLoadOp>::VMVXImportOpConversion;

  std::string getImportSuffix(IREE::VMVX::BufferLoadOp op) const override {
    return std::string(".1x") + getTypedTypeStr(op.value().getType());
  }
};

class ConvertVMVXBufferStoreOp
    : public VMVXImportOpConversion<IREE::VMVX::BufferStoreOp> {
 public:
  using VMVXImportOpConversion<
      IREE::VMVX::BufferStoreOp>::VMVXImportOpConversion;

  std::string getImportSuffix(IREE::VMVX::BufferStoreOp op) const override {
    return std::string(".1x") + getTypedTypeStr(op.value().getType());
  }
};
...
void populateVMVXToVMPatterns(MLIRContext *context,
  ...
  VMVX_IMPORT_OP(IREE::VMVX::InterfaceConstantOp, "vmvx.interface.constant");
  VMVX_IMPORT_OP(IREE::VMVX::InterfaceBindingOp, "vmvx.interface.binding");

  VMVX_IMPORT_OP(IREE::VMVX::BufferCopyOp, "vmvx.buffer.copy");
  patterns.insert<ConvertVMVXBufferLoadOp>(context, importSymbols,
                                           typeConverter, "vmvx.buffer.load");
  patterns.insert<ConvertVMVXBufferStoreOp>(context, importSymbols,
                                            typeConverter, "vmvx.buffer.store");
}

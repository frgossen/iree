...
Type VMVXDialect::parseType(DialectAsmParser &parser) const {
  StringRef typeName;
  ...
  if (!type) {
    parser.emitError(parser.getCurrentLocation())
        << "unknown VMVX type: " << typeName;
  }
  return type;
}

void VMVXDialect::printType(Type type, DialectAsmPrinter &p) const {
  if (type.isa<BufferType>()) {
    p << "buffer";
  } else if (type.isa<InterfaceType>()) {
    p << "interface";
  } else {
    llvm_unreachable("unknown VMVX type");
  }
}
